...
def pitcheranalyze(dataset):
    ...
    dataset.drop(["Blue_Spt_kda", f"Red_Spt_kda"], axis=1, inplace=True)
    ...
    dataset.drop(["Blue_Spt_creepScorePerTime", f"Red_Spt_creepScorePerTime"], axis=1, inplace=True)
    dataset.drop(["Blue_Spt_goldEarnedPerTime", f"Red_Spt_goldEarnedPerTime"], axis=1, inplace=True)
    for i in range(4):
        
        dataset[f"{numberToRoleName(i)}_kdaDiff"] = dataset[f"Blue_{numberToRoleName(i)}_kda"] - dataset[f"Red_{numberToRoleName(i)}_kda"]
        #dataset[f"{numberToRoleName(i)}_championDamageShareDiff"] = dataset[f"Blue_{numberToRoleName(i)}_championDamageShare"] - dataset[f"Red_{numberToRoleName(i)}_championDamageShare"]
